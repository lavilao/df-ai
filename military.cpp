        ...
        for (auto enemy_side : (*unit_side)->enemies)
        {
            if (enemy_side->conflict_level >= conflict_level::Lethal)
            {
                ...
                for (auto enemy_id : side->unit_ids)
                {
                    ...
                    {
                        if (Units::isSane(enemy) && Units::isCitizen(enemy, true))
                        {
                            citizen = enemy;
                            return true;
                        }
...
df::unit *AI::is_hunting_target(df::unit *u)
{
    ...
    {
        if (Units::isSane(c) && Units::isCitizen(c, true) && u->job.current_job && u->job.current_job->job_type == job_type::Hunt && c->job.hunt_target == u)
        {
            return c;
        }
    }
